...
function PipeFlowLogo() {
  return (
    <Link href="/" className="flex items-center gap-2.5 group">
      ...
      <span className="text-[15px] font-semibold tracking-tight text-gray-900">PipeFlow</span>
    </Link>
  );
}
    ...
    >
      <nav className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
        ...
        <button
          ...
          className="flex size-9 items-center justify-center rounded-lg text-gray-600 transition-colors hover:bg-gray-100 md:hidden"
          aria-label="Menu"
        >
      ...
      {/* mobile menu */}
      {mobileOpen && (
        <div className="border-t border-gray-100 bg-white/95 backdrop-blur-md px-6 py-4 md:hidden">
          <div className="flex flex-col gap-4">
            <a href="#funcionalidades" onClick={() => setMobileOpen(false)} className="text-sm font-medium text-gray-700">
              Funcionalidades
            </a>
            <a href="#precos" onClick={() => setMobileOpen(false)} className="text-sm font-medium text-gray-700">
              Preços
            </a>
            <div className="flex flex-col gap-2 pt-2 border-t border-gray-100">
              <Link href="/login" className="text-center rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700">
                Entrar
              </Link>
